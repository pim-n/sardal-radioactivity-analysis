...
plot(bomb_pulse)
data_C14$sardal <- data_C14$sardal - 0.7*bomb_pulse$F14C
...
$$
\text{mean(Temperature)} \sim CO}_2
$$
...
if(FULL_RUN) {
  ...
  model_C14_mvgam <- mvgam(formula = formula,
                           family = lognormal(),
                           data = data_C14,
                           trend_model = 'AR1',
                           burnin = 2000,
  ...
  save(model_C14_mvgam, file = "results/models/C14.mvgam.Rdata")
}
```
...
```{r}
mcmc_plot(model_C14_mvgam, variable='ar1[1]', type="areas")
```
